...
var __export = (all$1) => {
	let target = {};
	for (var name$2 in all$1) __defProp(target, name$2, {
		get: all$1[name$2],
		enumerable: true
	});
	return target;
...
try {
	...
	if (tsESLintParser = createRequire$1(path.join(cwd, "__placeholder__.js"))("@typescript-eslint/parser")) hasTypescriptEslintParser = true;
} catch {}
...
*/
function getSourceCode$1(context) {
...
*/
function getFilename$1(context) {
...
function getStaticAttributeValue(node, context) {
	...
	if (context && node.value?.type === "JSXExpressionContainer" && node.value.expression.type !== "JSXEmptyExpression") {
		const sourceCode = getSourceCode$1(context);
...
var missing_client_only_directive_value_default = createRule("missing-client-only-directive-value", {
	...
	create(context) {
		if (!getSourceCode$1(context).parserServices.isAstro) return {};
...
var no_deprecated_astro_canonicalurl_default = createRule("no-deprecated-astro-canonicalurl", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			...
			for (const { node: node$1, path: path$1 } of tracker.iterateGlobalReferences({ Astro: { canonicalURL: { [READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$1.join(".") }
			});
		} };
	}
});
...
var no_deprecated_astro_fetchcontent_default = createRule("no-deprecated-astro-fetchcontent", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			const tracker = new ReferenceTracker(sourceCode.getScope(node));
			for (const { node: node$1, path: path$1 } of tracker.iterateGlobalReferences({ Astro: { fetchContent: { [READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$1.join(".") },
				fix(fixer) {
...
var no_deprecated_astro_resolve_default = createRule("no-deprecated-astro-resolve", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			const tracker = new ReferenceTracker(sourceCode.getScope(node));
			for (const { node: node$1, path: path$1 } of tracker.iterateGlobalReferences({ Astro: { resolve: { [READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$1.join(".") }
			});
...
function loadModule(context, name$2) {
	const key = getSourceCode$1(context).ast;
	...
	if (mod) return mod;
	try {
		const cwd = getCwd$1(context);
		const relativeTo = path.join(cwd, "__placeholder__.js");
		return modules[name$2] = Module.createRequire(relativeTo)(name$2);
	} catch {
		return null;
	}
}
...
function transform(node, context) {
	...
	const code = getSourceCode$1(context).text.slice(...inputRange);
	const filename = `${getFilename$1(context)}.css`;
	try {
		const config = postcssLoadConfig.sync({
			cwd: getCwd$1(context) ?? process.cwd(),
			from: filename
		});
...
function transform$2(node, context) {
	...
	const inputRange = getContentRange(node);
	const code = getSourceCode$1(context).text.slice(...inputRange);
	const filename = `${getFilename$1(context)}.less`;
	try {
		let output;
		less.render(code, {
			sourceMap: {},
...
function transform$3(node, context) {
	...
	const inputRange = getContentRange(node);
	const code = getSourceCode$1(context).text.slice(...inputRange);
	const filename = `${getFilename$1(context)}.stylus`;
	try {
		let output;
...
function transformToStyleContentCSS(transform$4, context) {
	...
	function remapPosition(pos) {
		decoded = decoded ?? decode(transform$4.mappings);
...
function* extractClassListFromExpression(node, context) {
	...
	const staticValue = getStaticValue(node, getSourceCode$1(context).scopeManager.globalScope);
	if (staticValue) {
...
var prefer_split_class_list_default = createRule("prefer-split-class-list", {
	...
	create(context) {
		...
		return {
			Program(node) {
				const referenceTracker = new ReferenceTracker(getSourceCode$1(context).getScope(node));
...
function requireUserLocal(id) {
	try {
		const cwd = process.cwd();
		return createRequire$1(path.join(cwd, "__placeholder__.js"))(id);
	} catch {
		return null;
	}
}
...
const rules$1 = [...originalRules, ...buildA11yRules()];
...
function getEspree() {
	if (!espreeCache) {
		...
		if (linterPath) try {
			espreeCache = createRequire$1(linterPath)("espree");
		} catch {}
	}
	if (!espreeCache) espreeCache = requireUserLocal("espree");
	if (!espreeCache) espreeCache = __require("espree");
	return espreeCache;
}
...
function resolveParser() {
	for (const id of [
		...
		const parser$2 = toParserForESLint(requireUserLocal(id));
		if (!parser$2) continue;
		return parser$2;
	}
	try {
...
*/
function getPlugin() {
	const rules$2 = rules$1.reduce((obj, r) => {
...
const all = {};
for (const rule of rules$1.filter((rule$1) => rule$1.meta.docs.available() && !rule$1.meta.deprecated)) all[rule.meta.docs.ruleId] = "error";
...
var src_default = Object.assign(plugin, { configs });
...
//#endregion
export { configs, src_default as default, environments, meta, processors, rules };