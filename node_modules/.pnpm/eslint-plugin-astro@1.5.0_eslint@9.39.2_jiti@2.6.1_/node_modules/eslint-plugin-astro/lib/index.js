...
var __export = (all$2) => {
	let target = {};
	for (var name$2 in all$2) __defProp(target, name$2, {
		get: all$2[name$2],
		enumerable: true
	});
	return target;
...
let __eslint_community_eslint_utils = require("@eslint-community/eslint-utils");
...
let __jridgewell_sourcemap_codec = require("@jridgewell/sourcemap-codec");
...
try {
	...
	if (tsESLintParser = (0, module$1.createRequire)(path.default.join(cwd, "__placeholder__.js"))("@typescript-eslint/parser")) hasTypescriptEslintParser = true;
} catch {}
...
*/
function findAttribute(node, name$2) {
	const openingElement = node.openingElement;
	for (const attr of openingElement.attributes) {
		if (attr.type === "JSXSpreadAttribute") continue;
		if (getAttributeName(attr) === name$2) return attr;
	}
	return null;
}
/**
...
function getStaticAttributeValue(node, context) {
	if (node.value?.type === __typescript_eslint_types.AST_NODE_TYPES.Literal) return { value: node.value.value };
	if (context && node.value?.type === "JSXExpressionContainer" && node.value.expression.type !== "JSXEmptyExpression") {
		const sourceCode = getSourceCode(context);
		const staticValue = (0, __eslint_community_eslint_utils.getStaticValue)(node.value.expression, sourceCode.scopeManager.globalScope);
...
function isStringCallExpression(node) {
	if (node.type === __typescript_eslint_types.AST_NODE_TYPES.CallExpression) return node.callee.type === __typescript_eslint_types.AST_NODE_TYPES.Identifier && node.callee.name === "String";
	return false;
}
/** Checks whether given node is StringLiteral */
function isStringLiteral(node) {
	return node.type === __typescript_eslint_types.AST_NODE_TYPES.Literal && typeof node.value === "string";
}
...
function extractConcatExpressions(node, sourceCode) {
	...
	function processLeft(expr) {
		if (expr.type === __typescript_eslint_types.AST_NODE_TYPES.BinaryExpression) {
			if (!(0, __eslint_community_eslint_utils.isParenthesized)(expr, sourceCode) && expr.operator !== "*" && expr.operator !== "/") return extractConcatExpressions(expr, sourceCode);
		}
		return [expr];
	}
}
...
function getParenthesizedTokens(node, sourceCode) {
	...
	while ((maybeLeftParen = sourceCode.getTokenBefore(lastLeft)) && (maybeRightParen = sourceCode.getTokenAfter(lastRight)) && (0, __eslint_community_eslint_utils.isOpeningParenToken)(maybeLeftParen) && (0, __eslint_community_eslint_utils.isClosingParenToken)(maybeRightParen) && maybeLeftParen !== getParentSyntaxParen(node, sourceCode)) {
	...
	switch (parent.type) {
		...
		case "NewExpression":
			if (parent.arguments.length === 1 && parent.arguments[0] === node) return sourceCode.getTokenAfter(parent.callee, {
				...
				filter: __eslint_community_eslint_utils.isOpeningParenToken
			});
			return null;
...
var no_deprecated_astro_canonicalurl_default = createRule("no-deprecated-astro-canonicalurl", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			const tracker = new __eslint_community_eslint_utils.ReferenceTracker(sourceCode.getScope(node));
			for (const { node: node$1, path: path$5 } of tracker.iterateGlobalReferences({ Astro: { canonicalURL: { [__eslint_community_eslint_utils.READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$5.join(".") }
			});
		} };
	}
});
...
var no_deprecated_astro_fetchcontent_default = createRule("no-deprecated-astro-fetchcontent", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			const tracker = new __eslint_community_eslint_utils.ReferenceTracker(sourceCode.getScope(node));
			for (const { node: node$1, path: path$5 } of tracker.iterateGlobalReferences({ Astro: { fetchContent: { [__eslint_community_eslint_utils.READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$5.join(".") },
				fix(fixer) {
					if (node$1.type !== "MemberExpression" || node$1.computed) return null;
					return fixer.replaceText(node$1.property, "glob");
				}
			});
		} };
	}
...
var no_deprecated_astro_resolve_default = createRule("no-deprecated-astro-resolve", {
	...
	create(context) {
		...
		return { "Program:exit"(node) {
			const tracker = new __eslint_community_eslint_utils.ReferenceTracker(sourceCode.getScope(node));
			for (const { node: node$1, path: path$5 } of tracker.iterateGlobalReferences({ Astro: { resolve: { [__eslint_community_eslint_utils.READ]: true } } })) context.report({
				node: node$1,
				messageId: "deprecated",
				data: { name: path$5.join(".") }
			});
...
* Load module
*/
function loadModule(context, name$2) {
	...
	const mod = modules[name$2];
	if (mod) return mod;
	try {
		const cwd = getCwd(context);
		const relativeTo = path.default.join(cwd, "__placeholder__.js");
		return modules[name$2] = module$1.default.createRequire(relativeTo)(name$2);
	} catch {
		return null;
	}
}
...
* Transpile with sass
*/
function transform$1(node, context, type) {
...
* Transpile with less
*/
function transform$2(node, context) {
...
function getStyleContentCSS(node, context) {
	...
	if (lang === "postcss") transform$4 = transform(node, context);
	else if (lang === "scss" || lang === "sass") transform$4 = transform$1(node, context, lang);
	else if (lang === "less") transform$4 = transform$2(node, context);
	else if (lang === "styl" || lang === "stylus") transform$4 = transform$3(node, context);
...
function transformToStyleContentCSS(transform$4, context) {
	const sourceCode = getSourceCode(context);
	let outputLocs = null;
	let inputLocs = null;
	let decoded = null;
	return {
		css: transform$4.output,
		remap: (index) => {
			outputLocs = outputLocs ?? new LinesAndColumns(transform$4.output);
			inputLocs = inputLocs ?? new LinesAndColumns(sourceCode.text.slice(...transform$4.inputRange));
			const inputCodePos = remapPosition(outputLocs.getLocFromIndex(index));
			return inputLocs.getIndexFromLoc(inputCodePos) + transform$4.inputRange[0];
		}
	};
	/** Remapping source position */
	function remapPosition(pos) {
		decoded = decoded ?? (0, __jridgewell_sourcemap_codec.decode)(transform$4.mappings);
		...
		if (!lineMaps?.length) {
			for (let line = pos.line - 1; line >= 0; line--) {
				...
				if (prevLineMaps?.length) {
					const [, , sourceCodeLine$1, sourceCodeColumn$1] = prevLineMaps[prevLineMaps.length - 1];
					return {
						line: sourceCodeLine$1 + 1,
						column: sourceCodeColumn$1
					};
				}
			}
			return {
		...
		for (let index = 0; index < lineMaps.length - 1; index++) {
			const [generateCodeColumn$1, , sourceCodeLine$1, sourceCodeColumn$1] = lineMaps[index];
			if (generateCodeColumn$1 <= pos.column && pos.column < lineMaps[index + 1][0]) return {
				line: sourceCodeLine$1 + 1,
				column: sourceCodeColumn$1 + (pos.column - generateCodeColumn$1)
			};
		}
...
var no_unused_css_selector_default = createRule("no-unused-css-selector", {
	...
	create(context) {
		...
		return {
			JSXElement(node) {
				const name$2 = getElementName(node);
				if (name$2 === "Fragment" || name$2 === "slot") return;
				if (name$2 === "style" && !findAttribute(node, "is:global")) styles.push(node);
				const tree = {
					parent: currTree,
					node,
					childElements: []
...
function tagNodeToJSXElementMatcher(selector) {
	const name$2 = selector.value;
	return (element) => {
		return getElementName(element.node) === name$2;
	};
}
/**
...
function extractClassList(node, context) {
	if (node.value?.type === __typescript_eslint_types.AST_NODE_TYPES.Literal) return { classList: [String(node.value.value)] };
...
function* extractClassListFromExpression(node, context) {
	if (node.type === __typescript_eslint_types.AST_NODE_TYPES.ArrayExpression) {
		for (const element of node.elements) {
			if (element == null) continue;
			if (element.type === __typescript_eslint_types.AST_NODE_TYPES.SpreadElement) yield* extractClassListFromExpression(element.argument, context);
			else yield* extractClassListFromExpression(element, context);
		}
		return;
	}
	if (node.type === __typescript_eslint_types.AST_NODE_TYPES.ObjectExpression) {
		for (const prop of node.properties) if (prop.type === __typescript_eslint_types.AST_NODE_TYPES.SpreadElement) yield* extractClassListFromExpression(prop.argument, context);
		else if (!prop.computed) if (prop.key.type === __typescript_eslint_types.AST_NODE_TYPES.Literal) yield String(prop.key.value);
		else yield prop.key.name;
		else yield* extractClassListFromExpression(prop.key, context);
		return;
	}
	const staticValue = (0, __eslint_community_eslint_utils.getStaticValue)(node, getSourceCode(context).scopeManager.globalScope);
	if (staticValue) {
...
let CSSTokenType = /* @__PURE__ */ function(CSSTokenType$1) {
	CSSTokenType$1["quoted"] = "Quoted";
	CSSTokenType$1["block"] = "Block";
	CSSTokenType$1["line"] = "Line";
	CSSTokenType$1["word"] = "Word";
	CSSTokenType$1["punctuator"] = "Punctuator";
	return CSSTokenType$1;
}({});
/**
...
var no_unused_define_vars_in_style_default = createRule("no-unused-define-vars-in-style", {
	...
	create(context) {
		...
		return { "JSXElement > JSXOpeningElement[name.type='JSXIdentifier'][name.name='style']"(node) {
			...
			if (!defineVars.value || defineVars.value.type !== __typescript_eslint_types.AST_NODE_TYPES.JSXExpressionContainer || defineVars.value.expression.type !== __typescript_eslint_types.AST_NODE_TYPES.ObjectExpression) return;
			...
			const definedVars = defineVars.value.expression.properties.filter((prop) => prop.type === __typescript_eslint_types.AST_NODE_TYPES.Property).map((prop) => ({
				prop,
				name: (0, __eslint_community_eslint_utils.getPropertyName)(prop, sourceCode.getScope(node))
			})).filter((data) => Boolean(data.name));
			if (!definedVars.length) return;
			const lang = node.attributes.find((attr) => getAttributeName(attr) === "lang");
			const langValue = lang && lang.value && lang.value.type === __typescript_eslint_types.AST_NODE_TYPES.Literal && lang.value.value;
	...
	create(context) {
		...
		class NewObjectProps {
			...
			fixObject({ fixer, object }) {
				...
				let text;
				if ((0, __eslint_community_eslint_utils.isCommaToken)(maybeComma)) text = this.toPropsString();
		...
		*/
		function endsWithSpace(elements) {
			for (let i = elements.length - 1; i >= 0; i--) {
				...
				if (strings == null) {
					if (valueNode.type === __typescript_eslint_types.AST_NODE_TYPES.TemplateLiteral) {
		...
		*/
		...
		function verifyAttr(attr) {
			...
			if (!attr.value || attr.value.type !== __typescript_eslint_types.AST_NODE_TYPES.JSXExpressionContainer || attr.value.expression.type === __typescript_eslint_types.AST_NODE_TYPES.JSXEmptyExpression) return;
			...
			for (const element of extractElements(expression)) visitElementExpression(element.node, {
				...
				*fixArrayElement(data) {
					...
					if ((0, __eslint_community_eslint_utils.isCommaToken)(maybeComma)) {
				...
				*fixExpression(data) {
					if (element.array) {
						...
						if (object) {
							...
							const tokens = getParenthesizedTokens(element.node, sourceCode);
							const maybeComma = sourceCode.getTokenAfter(tokens.right);
							if ((0, __eslint_community_eslint_utils.isCommaToken)(maybeComma)) yield data.fixer.removeRange(maybeComma.range);
							else {
								const maybeBeforeComma = sourceCode.getTokenBefore(tokens.left);
								if ((0, __eslint_community_eslint_utils.isCommaToken)(maybeBeforeComma)) yield data.fixer.removeRange(maybeBeforeComma.range);
							}
							yield data.newProps.fixObject({
								fixer: data.fixer,
								object
			...
			*/
			function findClosestObject(array, target) {
				...
				for (let index$1 = 0; index$1 < length; index$1++) {
					const after = afterElements[index$1];
					if (after?.type === __typescript_eslint_types.AST_NODE_TYPES.ObjectExpression) return after;
					const before = beforeElements[index$1];
					if (before?.type === __typescript_eslint_types.AST_NODE_TYPES.ObjectExpression) return before;
				}
				return null;
			}
			/** Visit expression */
			function visitElementExpression(node, state) {
				if (node.type === __typescript_eslint_types.AST_NODE_TYPES.ConditionalExpression) verifyConditionalExpression(node, state);
				else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.TemplateLiteral) {
					const quasis = [...node.quasis];
					let beforeQuasiWk = quasis.shift();
					for (const expression$1 of node.expressions) {
						const beforeQuasi = beforeQuasiWk;
						const afterQuasi = quasis.shift();
						visitElementExpression(expression$1, {
							beforeIsWord() {
								const beforeElements = [];
								const targetIndex = node.expressions.indexOf(expression$1);
				...
				} else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.CallExpression) {
					if (isStringCallExpression(node) && node.arguments[0] && node.arguments[0].type !== __typescript_eslint_types.AST_NODE_TYPES.SpreadElement) visitElementExpression(node.arguments[0], {
					...
					else if (node.callee.type === __typescript_eslint_types.AST_NODE_TYPES.MemberExpression && (0, __eslint_community_eslint_utils.getPropertyName)(node.callee) === "trim") {
				...
				} else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.BinaryExpression) {
					const elements = extractConcatExpressions(node, sourceCode);
					if (!elements) return;
					for (const expression$1 of elements) visitElementExpression(expression$1, {
						beforeIsWord() {
							const index = elements.indexOf(expression$1);
						...
						*fixExpression(data) {
							...
							const tokens = getParenthesizedTokens(expression$1, sourceCode);
		...
		function extractElements(node) {
			if (node.type === __typescript_eslint_types.AST_NODE_TYPES.ArrayExpression) {
				const result = [];
				for (const element of node.elements) {
					if (!element || element.type === __typescript_eslint_types.AST_NODE_TYPES.SpreadElement) continue;
	...
	create(context) {
		...
		function verifyAttr(attr) {
			...
			verifyExpression(expression, function* (fixer) {
				if (expression.type === __typescript_eslint_types.AST_NODE_TYPES.ArrayExpression) return;
		...
		function verifyExpression(node, transformArray, call) {
			if (node.type === __typescript_eslint_types.AST_NODE_TYPES.TemplateLiteral) {
			...
			} else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.BinaryExpression) verifyBinaryExpression(node, transformArray);
			else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.ArrayExpression) {
				for (const element of node.elements) if (element) verifyExpression(element, transformArray);
			} else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.Literal) {
				...
				if (node.callee.type === __typescript_eslint_types.AST_NODE_TYPES.MemberExpression && (0, __eslint_community_eslint_utils.getPropertyName)(node.callee) === "trim") verifyExpression(node.callee.object, transformArray, ".trim()");
			}
		}
		...
		function verifyClsx(clsxCall) {
			if (clsxCall.node.type !== __typescript_eslint_types.AST_NODE_TYPES.CallExpression) return;
			const callNode = clsxCall.node;
			const parent = callNode.parent;
			if (!parent || parent.type !== __typescript_eslint_types.AST_NODE_TYPES.JSXExpressionContainer || parent.expression !== callNode) return;
			const parentParent = parent.parent;
			if (!parentParent || parentParent.type !== __typescript_eslint_types.AST_NODE_TYPES.JSXAttribute || parentParent.value !== parent || getAttributeName(parentParent) !== "class:list") return;
			context.report({
		...
		return {
			Program(node) {
				const referenceTracker = new __eslint_community_eslint_utils.ReferenceTracker(getSourceCode(context).getScope(node));
				for (const call of referenceTracker.iterateEsmReferences({ clsx: { [__eslint_community_eslint_utils.ReferenceTracker.CALL]: true } })) verifyClsx(call);
			},
			JSXAttribute: verifyAttr,
			AstroTemplateLiteralAttribute: verifyAttr
		};
...
function isStringType(node) {
	if (node.type === __typescript_eslint_types.AST_NODE_TYPES.Literal) return typeof node.value === "string";
	else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.TemplateLiteral) return true;
	else if (node.type === __typescript_eslint_types.AST_NODE_TYPES.BinaryExpression) return isStringType(node.left) || isStringType(node.right);
...
var fix_tracker_default = FixTracker;
	...
	create(context) {
		...
		*/
		function isRedundantSemi(semiToken) {
			...
			return !nextToken || (0, __eslint_community_eslint_utils.isClosingBraceToken)(nextToken) || (0, __eslint_community_eslint_utils.isSemicolonToken)(nextToken);
		}
		/**
		...
		*/
		function isEndOfArrowBlock(lastToken) {
			if (!(0, __eslint_community_eslint_utils.isClosingBraceToken)(lastToken)) return false;
		...
		*/
		function checkForSemicolon(node) {
			const isSemi = (0, __eslint_community_eslint_utils.isSemicolonToken)(sourceCode.getLastToken(node));
			if (never) {
				const nextToken = sourceCode.getTokenAfter(node);
		...
		return { JSXElement(node) {
			...
			*/
			...
			const parts = attributes.reduce((accumulator, attribute) => {
				...
				const name$2 = typeof attribute.name.name === "string" ? attribute.name.name : sourceCode.text.slice(...attribute.name.range);
			...
			*/
			function formatName(name$2) {
				return context.options[0]?.ignoreCase === false ? name$2 : name$2.toLowerCase();
			}
		} };
	}
});
...
function requireUserLocal(id) {
	try {
		const cwd = process.cwd();
		return (0, module$1.createRequire)(path.default.join(cwd, "__placeholder__.js"))(id);
	} catch {
		return null;
	}
}
...
function isLinterPath(p) {
	return p.includes(`eslint${path.default.sep}lib${path.default.sep}linter${path.default.sep}linter.js`);
}
/**
...
function getEspree() {
	if (!espreeCache) {
		...
		if (linterPath) try {
			espreeCache = (0, module$1.createRequire)(linterPath)("espree");
		} catch {}
	}
...
function resolveParser() {
	for (const id of [
		...
		const parser$1 = toParserForESLint(requireUserLocal(id));
...
var ClientScript = class {
	...
	initBlock() {
		...
		const remapRange = (range) => {
			const startLoc$1 = textLocs.getLocFromIndex(range[0]);
			...
			const remappedStartLoc = remapLoc(startLoc$1);
		...
		return {
			...
			remapMessage(message) {
				...
				if (typeof message.endLine === "number" && typeof message.endColumn === "number") {
					const loc$1 = remapLoc({
						line: message.endLine,
						column: message.endColumn
					});
					message.endLine = loc$1.line;
					message.endColumn = loc$1.column;
				}
				if (message.fix) {
	...
	extractDefineVars() {
		...
		if (expression.type !== __typescript_eslint_types.AST_NODE_TYPES.ObjectExpression) return [];
		const startLoc = this.parsed.getLocFromIndex(valueStart + 1);
		return expression.properties.filter((p) => p.type === __typescript_eslint_types.AST_NODE_TYPES.Property).filter((p) => !p.computed).map((p) => {
			return {
				name: p.key.type === __typescript_eslint_types.AST_NODE_TYPES.Identifier ? p.key.name : p.key.value,
				loc: {
...
function preprocess(code, filename, virtualFileExt) {
	if (filename) {
		...
		parsed.walk(parsed.result.ast, (node) => {
			if (node.type === "element" && node.name === "script" && node.children.length && !node.attributes.some(({ name: name$2, value }) => name$2 === "type" && /json$|importmap/i.test(value))) shared.addClientScript(code, node, parsed);
		});
...
function buildFlatConfigs() {
	const configs$1 = {};
	for (const configName of a11yConfigKeys) Object.defineProperty(configs$1, `jsx-a11y-${configName}`, {
		enumerable: true,
		get() {
...
function buildLegacyConfigs() {
	const baseExtend$1 = "plugin:astro/base";
	const configs$1 = {};
	for (const configName of a11yConfigKeys) Object.defineProperty(configs$1, `jsx-a11y-${configName}`, {
		enumerable: true,
		get() {
	...
	*/
	function getProxyNode(node, overrides) {
		...
		if (node.type === "JSXAttribute") {
			...
			if (converted) cache$2.name = getProxyNode(node.name, {
				type: "JSXIdentifier",
				namespace: null,
				name: converted
			});
		...
		return new Proxy(node, { get(_t, key) {
			if (key in cache$2) return cache$2[key];
			const data = node[key];
			if (isNode(data)) return cache$2[key] = getProxyNode(data);
			if (Array.isArray(data)) return cache$2[key] = data.map((e) => isNode(e) ? getProxyNode(e) : e);
			return data;
		} });
	}
...
const all$1 = {};
for (const rule of rules.filter((rule$1) => rule$1.meta.docs.available() && !rule$1.meta.deprecated)) all$1[rule.meta.docs.ruleId] = "error";
var all_default = {
...
var recommended_default$1 = [...base_default, {
...
const all = {};
for (const rule of rules.filter((rule$1) => rule$1.meta.docs.available() && !rule$1.meta.deprecated)) all[rule.meta.docs.ruleId] = "error";
var all_default$1 = [...recommended_default$1, { rules: {
	...all,
	...recommended_default$1[recommended_default$1.length - 1].rules
} }];
...
function buildCjsConfigs() {
	const cjsConfigs = {
		...
		"flat/recommended": recommended_default$1,
		"flat/all": all_default$1,
